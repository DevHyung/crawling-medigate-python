...
def initExcel(filename):
    header1 = ['지역','분류','과목','기관','기관명','제목','조회','작성일','마감','Phone','Email','URL']
    wb = Workbook()
    ws1 = wb.worksheets[0]
    ws1.append(header1)
    style_range(ws1, 'A1:L1', border=border, fill=fill,font=font, alignment=ali)
    wb.save(filename)
def saveExcel(datalist,imagelist,alist,pl,el,filename):
    ...
    for rowidx in range(len(datalist)):
        ...
        for td in datalist[rowidx]:
            ...
            if idx==0 or idx == 4:
                ...
                ws1.cell(row=startrow, column=idx + 1, value=td)
            idx += 1
        ws1.cell(row=startrow, column=10, value=pl[rowidx])
        ws1.cell(row=startrow, column=11, value=el[rowidx])
        ws1.cell(row=startrow, column=12, value=alist[rowidx])
        startrow += 1
    colidx = 0
    for col in ws1.columns:
        column = col[0].column  # Get the column name
        if colidx == 4 or colidx == 9 or colidx == 10:
    ...
    try:
        for pagenum in range(startpage,endpage+1):
            ...
            for detail in alist:
                ...
                time.sleep(cycle)
                bs4 = BeautifulSoup(driver.page_source,'lxml')
                td = bs4.find('td',class_="cont_info cell_2")
                plist = td.find_all('p',class_="txt")[1:]

                try:
                    phone = str(plist[0]).split('휴대폰 (')[1].split(')')[0].strip()
                    phonelist.append(phone)
                except:
                    phonelist.append('')
                try:
                    email = plist[2].get_text().strip()
                ...
                picidx+=1
                alistidx+=1
            saveExcel(datalist,imagelist,alist,phonelist,emaillist,FILENAME)
            print("\t>>> 엑셀에 저장완료...")
